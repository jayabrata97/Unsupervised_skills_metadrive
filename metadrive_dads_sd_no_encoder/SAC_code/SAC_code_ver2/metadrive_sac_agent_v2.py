...
from metadrive_networks_v2 import ActorNetwork, CriticNetwork, DoubleCriticNetwork, ValueNetwork
...
#         self.critic_1 = CriticNetwork(lr, obs_dims, n_actions, latent_dims, critic_layers, features_dim)
#         self.critic_2 = CriticNetwork(lr, obs_dims, n_actions, latent_dims, critic_layers, features_dim)
#         self.value = ValueNetwork(lr, obs_dims, n_actions, latent_dims, critic_layers, features_dim)
#         self.target_value = ValueNetwork(lr, obs_dims, n_actions, latent_dims, critic_layers, features_dim)
...
#         reward = T.unsqueeze(T.tensor(reward, dtype=T.float),1).to(self.critic_1.device)
#         done = T.unsqueeze(T.from_numpy(done).float(),1).to(self.critic_1.device)
...
#         skills = T.tensor(skills, dtype=T.float, device=self.critic_1.device)

#         value = self.value(state)
#         value_ = self.target_value(state_)
#         value_[done] = 0.0

#         actions, log_probs = self.actor.sample_normal(state_, skills, reparameterize=False)
...
#         actions, log_probs = self.actor.sample_normal(state, skills, reparameterize=True)
#         q1_new_policy = self.critic_1.forward(state, actions, skills)
#         q2_new_policy = self.critic_2.forward(state, actions, skills)
#         critic_value = T.min(q1_new_policy, q2_new_policy)
...
class SacAgent():
    def __init__(self, 
        ...
        self.critic = DoubleCriticNetwork(lr, obs_dims, n_actions, latent_dims, critic_layers, features_dim)
